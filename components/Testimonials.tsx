...
const TestimonialCard: React.FC<{ testimonial: typeof testimonials[0], onClick: () => void }> = ({ testimonial, onClick }) => {
    return (
        <div 
            className="bg-gradient-to-br from-orange-50/50 to-white p-2 sm:p-3 md:p-4 lg:p-6 rounded-md sm:rounded-lg md:rounded-xl shadow-sm relative transition-all duration-300 hover:shadow-lg border border-orange-100/50 cursor-pointer h-full"
            onClick={onClick}
        >
            {/* Quote Icon */}
...
const Testimonials: React.FC<TestimonialsProps> = ({ onTestimonialClick }) => {
          ...
          <div className="grid md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            {displayedTestimonials.map((testimonial, index) => (
              <TestimonialCard 
                ...
                onClick={() => onTestimonialClick && onTestimonialClick(testimonial.id)}
              />
            ))}
          </div>
