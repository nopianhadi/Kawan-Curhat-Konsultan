...
const ServiceItem: React.FC<{ service: typeof services[0]; index: number; onClick: () => void }> = ({ service, index, onClick }) => {
    ...
    return (
        <div 
            ref={ref} 
            className={`flex flex-col sm:flex-row items-start gap-3 sm:gap-4 md:gap-6 bg-white rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 cursor-pointer ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            style={{transitionDelay: `${index * 100}ms`}}
            onClick={onClick}
        >
            <div className="flex-shrink-0 w-full sm:w-40 md:w-48 overflow-hidden rounded-lg">
                <div className="relative w-full" style={{ paddingBottom: '60%' }}>
                    <img src={service.imgSrc} alt={service.name} className="absolute inset-0 w-full h-full object-cover"/>
                </div>
            </div>
            ...
            <div className="flex-shrink-0 self-center sm:self-start">
                <div className="w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-brand-accent flex items-center justify-center text-white hover:bg-opacity-90 transition-all">
    ...
    <section id={id} className="bg-white py-6 sm:py-12 md:py-16 lg:py-24 overflow-hidden">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="text-center mt-4 sm:mt-6 md:mt-8 lg:mt-12">
          <button className="inline-block bg-brand-accent text-white font-semibold px-5 py-2.5 sm:px-6 sm:py-3 md:px-8 md:py-3 text-xs sm:text-sm md:text-base rounded-lg shadow-md hover:shadow-lg hover:bg-opacity-90 transition-all focus:outline-none focus:ring-2 focus:ring-brand-accent">
            Lihat Semua Layanan
          </button>
        </div>
      </div>
